...
/**
 ******************************************************************************
 * @file           : 'exemple_aht20_polling_error_handling.c'
 * @brief          : Exemple gestion d'erreurs et diagnostic AHT20
 * @date           : 2026-02-12
 ******************************************************************************
 * @description
 * Démontre la gestion avancée des erreurs avec le capteur AHT20 :
 * - Utilisation de AHT20_GetStatus() pour vérifier état du capteur
 * - Détection des erreurs de busy, checksum, calibration
 * - Affichage détaillé avec AHT20_StatusToString()
 * - Retry automatique en cas d'erreur temporaire
 * - Monitoring du statut brut (bit busy, bit calibration)
 *
...
typedef struct {
    ...
    uint32_t success_reads;
    uint32_t error_busy;
    uint32_t error_checksum;
    uint32_t error_i2c;
    uint32_t error_other;
} AHT20_Stats_t;
/* USER CODE END PTD */
...
/* USER CODE BEGIN PFP */
static void PrintStatusByte(uint8_t status_byte);
...
/**
 * @brief Affiche les bits du registre status AHT20
 * @param status_byte Octet de statut brut (0x00-0xFF)
 */
static void PrintStatusByte(uint8_t status_byte)
{
    printf("  [Status Byte: 0x%02X]\r\n", status_byte);
    printf("    Bit 7 (Busy)       : %s\r\n", (status_byte & 0x80) ? "1 (OCCUPÉ)" : "0 (PRÊT)");
    printf("    Bit 6-4 (Mode)     : 0x%X\r\n", (status_byte >> 4) & 0x07);
    printf("    Bit 3 (Calibration): %s\r\n", (status_byte & 0x08) ? "1 (CALIBRÉ)" : "0 (NON CALIBRÉ!)");
    printf("    Bit 2-0 (Reserved) : 0x%X\r\n", status_byte & 0x07);
}

/**
 * @brief Affiche les statistiques de mesure
 */
static void PrintStatistics(void)
{
  printf("\r\n========================================\r\n");
  printf("  Statistiques AHT20\r\n");
  printf("========================================\r\n");
    printf("  Lectures totales : %lu\r\n", stats.total_reads);
    printf("  Succès           : %lu (%.1f%%)\r\n",
           stats.success_reads,
           stats.total_reads > 0 ? (stats.success_reads * 100.0f) / stats.total_reads : 0.0f);
    printf("  Erreurs BUSY     : %lu\r\n", stats.error_busy);
    printf("  Erreurs CHECKSUM : %lu\r\n", stats.error_checksum);
    printf("  Erreurs I2C      : %lu\r\n", stats.error_i2c);
    printf("  Erreurs autres   : %lu\r\n", stats.error_other);
  printf("========================================\r\n\r\n");
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    printf("  AHT20 - Gestion d'erreurs\r\n");
    printf("========================================\r\n\r\n");
  ...
  /* Lecture statut registre pour diagnostic visuel (bit Busy et bit CAL) */
  status = AHT20_GetStatus(&haht20, &status_byte);
  if (status == AHT20_OK) {
      printf("INFO  Statut initial capteur:\r\n");
