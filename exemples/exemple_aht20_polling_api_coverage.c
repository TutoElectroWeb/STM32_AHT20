...
/**
  ******************************************************************************
  * @file           : exemple_aht20_polling_api_coverage.c
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define LOG_NAME            "exemple_aht20_polling_api_coverage"  ///< Identifiant log série
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (tick == AHT20_TICK_DATA_READY) {
       ...
       * Note : AHT20_Async_Tick appelle GetData en interne → data_ready_flag est déjà
       *        à false à ce stade. ClearFlags reste utile si error_flag a été levé. */
